...
function ShopNav({ currentTab, onTabChange }) {
  ...
  return (
    <nav className="flex flex-col sm:flex-row items-stretch gap-2 sm:gap-4 mb-8 mt-8">
      ...
      >
        <div className="flex flex-col flex-1 items-center">
          <span className="text-white text-sm sm:text-base">{t('store.navigation.sales.title')}</span>
          <span className="text-[#ff6b00] text-lg sm:text-2xl font-bold">{t('store.navigation.sales.subtitle')}</span>
        </div>
